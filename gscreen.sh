...
if [ "$before" == "true" ]; then
    echo $filepath/$filename | dbus-launch wl-copy
fi

# Take screenshot and copy to clipboard
if [ "$entire" == "true" ]; then # take regular screenshot
    gnome-screenshot -f "$filepath/$filename" | dbus-launch wl-copy
elif [ "$window" == "true" ]; then # take screenshot of window
    gnome-screenshot -w -f "$filepath/$filename" | dbus-launch wl-copy
else # take selective screenshot
    gnome-screenshot -a -f "$filepath/$filename" | dbus-launch wl-copy
fi
